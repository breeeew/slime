...
public class BluetoothUIManager extends Activity{
	...
	private BluetoothSrvice mBtService = null;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        serverButton.setText("Подключиться к игре");
        clientButton.setText("Создать игру");
...
//				Toast.makeText(getApplicationContext(), new String(addHashToBytes(buffer)).substring(getBuffer_size()), Toast.LENGTH_LONG).show();
    ...
    public synchronized void onResume() {
        ...
        if (mBtService != null) {
            ...
            if (mBtService.getState() == BluetoothSrvice.STATE_NONE) {
    ...
    private void setupBtService() {
        ...
        mBtService = new BluetoothSrvice(this, mHandler);
    ...
    private final Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            case MESSAGE_STATE_CHANGE:
                switch (msg.arg1) {
                case BluetoothSrvice.STATE_CONNECTED:
                    //mTitle.setText(R.string.title_connected_to);
                    //mTitle.append(mConnectedDeviceName);
                    //mConversationArrayAdapter.clear();
                	Toast.makeText(getApplicationContext(),"Подключен к " + mConnectedDeviceName, Toast.LENGTH_LONG).show();
                    break;
                case BluetoothSrvice.STATE_CONNECTING:
                    //mTitle.setText(R.string.title_connecting);
                	Toast.makeText(getApplicationContext(), "Идет подключение..", Toast.LENGTH_LONG).show();
                    break;
                case BluetoothSrvice.STATE_LISTEN:
                	break;
                case BluetoothSrvice.STATE_NONE:
    ...
    private void sendMessage(byte[] message) {
        if (mBtService.getState() != BluetoothSrvice.STATE_CONNECTED) {
